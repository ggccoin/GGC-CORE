...
#if defined(NDEBUG)
#error "Globalgame Coin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uggc) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    else if (nHeight > 200 && nHeight <= 250)
        nSubsidy = 35000 * COIN;
    else if (nHeight > 251 && nHeight <= 5032894)
        nSubsidy = 7.6 * COIN;
    else if (nHeight > 5032894 && nHeight <= 5032895)
        nSubsidy = 6 * COIN;
    else 
        nSubsidy = 0 * COIN;
...
{
    RenameThread("globalgamecoin-scriptch");
